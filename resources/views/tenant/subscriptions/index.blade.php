        ...
        <div class="col-md-8">
            ...
            <div class="card">
                <div class="card-header text-center">
                    <h5>Subscription for {{ tenant('id') }}</h5>
                </div>
                ...
                <div class="card-body">
                    ...
                    @else
                        ...
                        @else
                            ...
                            {{-- Cancel buttons when active --}}
                            @if($subscription->isActive())
                                <div class="d-flex justify-content-center gap-2">
                                    {{-- Cancel at period end --}}
                                    <form method="POST"
                                          action="{{ route('tenant.subscriptions.cancel.period_end', [
                                              'tenant'       => tenant('id'),
                                        ...
                                        <button type="submit"
                                                ...
                                                onclick="return confirm('Keep access until the end of the current billing period, then stop?');">
                                            Cancel at Period End
                                        </button>
                                    </form>
                                    ...
                                    {{-- Cancel immediately --}}
                                    <form method="POST"
                                          action="{{ route('tenant.subscriptions.cancel.now', [
                                              'tenant'       => tenant('id'),
                                              'subscription' => $subscription->id,
                                          ]) }}">
                                        @csrf
