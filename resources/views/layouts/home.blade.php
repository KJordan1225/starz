...
<body class="d-flex flex-column min-vh-100">
    ...
    <nav class="navbar navbar-expand-lg topbar navbar-dark sticky-top">
        <div class="container-fluid px-2 px-sm-3">
            ...
            {{-- Center + Right: TOP NAV CONTENT (collapses on mobile) --}}
            <div class="collapse navbar-collapse mt-2 mt-lg-0" id="topnavLinks">
                ...
                <ul class="navbar-nav mx-auto mb-2 mb-lg-0 text-center text-lg-start">
                    ...
                    <li class="nav-item">
                        <a class="nav-link text-white{{ request()->routeIs('guestr.about') ? ' active fw-semibold' : '' }}"
                           href="#"
                           style="font-size:20px;">
                            Plans
                        </a>
                    ...
                    <li class="nav-item">
                        <a class="nav-link text-white{{ request()->routeIs('guest.contact') ? ' active fw-semibold' : '' }}"
                           href="#"
                           style="font-size:20px;">
                            Contact
                        </a>
                ...
                <div class="ms-lg-auto d-flex flex-column flex-sm-row align-items-sm-center gap-2 gap-sm-3 text-center text-sm-start">
                    @auth
                        <span class="text-white-50 small">
                            Logged in: <strong class="text-white">{{ auth()->user()->name }}</strong>
                        </span>
                        <form method="POST"
                              action="{{ route('logout') }}"
                              class="m-0">
                            @csrf
                            <button class="btn btn-sm btn-light w-100 w-sm-auto" type="submit">
                                Log out
                            </button>
                        </form>
                    @else
                ...
                <aside class="sidebar sticky-top" style="top: 1rem;">
                    ...
                    <div class="card shadow-sm mt-3">
                        <div class="card-body small text-muted">
                            <div class="mb-1">
                                Brand Primary: <code>{{ $branding['primary_color'] ?? '' }}</code>
                            </div>
                            <div class="mb-1">
                                Accent: <code>{{ $branding['accent_color'] ?? '' }}</code>
                            </div>
                        </div>
                    </div>
                </aside>
